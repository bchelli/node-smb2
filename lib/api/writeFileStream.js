...
module.exports = 
  ...
  try {
    var file = await createFile(filename, connection);
    await setFileSize(file, fileLength, connection);
    var writeStream = new SambaWriter(connection, file);
...
class SambaWriter extends Stream.Writable {
  ...
  writeBuffer(chunk, currentOffset, done) {
    ...
    pr.then(() => {
      var index = this.promises.findIndex(x => x === pr);
      if(index >= 0) {
  ...
  _write(chunk, encoding, done) {
    ...
    var pr = this.writeBuffer(buffer, currentOffset, done);
